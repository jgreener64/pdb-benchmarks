# Test which new/modified PDB entries error on PDB/mmCIF parsers
...
start = now()
basedir = "."
ad, mo, ob = pdbrecentchanges()

outstrs = ["Checking new/modified PDB entries at $(now())",
        "Checking $(length(ad)) new and $(length(mo)) modified entries"]

for p in sort(collect(Set([ad..., mo...])))
    try
...
open("$basedir/recentpdb_jl_$datestr.txt", "w") do f
    for l in outstrs
        println(f, l)
    end
end
